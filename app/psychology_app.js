document.addEventListener('DOMContentLoaded', () => {
    ...
    if (typeof psychologyData === 'undefined') {
        console.error("psychology_data.js 로드 실패");
        return;
    }
    ...
    psychologyData.forEach(item => {
        ...
        // ★ 디자인: Rose(분홍) 테마 적용 ('psycho' 클래스 사용)
        container.innerHTML += `
            <div class="group bg-white p-5 rounded-2xl border border-slate-100 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 cursor-pointer flex flex-col justify-between h-40" 
                ...
                <div class="flex justify-between items-start">
                    ...
                    <span class="text-[10px] font-bold px-2 py-1 rounded-md border bg-psycho-50 text-psycho-600 border-psycho-100">
                        Psyche
                    </span>
                </div>
                <div class="text-center">
                    <div class="font-bold text-slate-800 text-lg group-hover:text-psycho-600 transition-colors mb-1 leading-tight truncate px-1">${item.w}</div>
                ...
                <div class="text-center opacity-0 group-hover:opacity-100 transition-opacity">
                        <i class="ph-bold ph-heart text-psycho-400"></i>
                </div>
            </div>
        `;
    });
...
function startQuiz() {
    pool = [...psychologyData].sort(() => 0.5 - Math.random()).slice(0, 10);
...
function showQ() {
    ...
    while(choices.length < 4) {
        let randomWord = psychologyData[Math.floor(Math.random()*psychologyData.length)].w;
    ...
    choices.sort(() => 0.5 - Math.random()).forEach(choice => {
        ...
        // ★ 버튼 스타일: Rose 테마
        btn.className = "w-full text-left p-5 rounded-xl border border-slate-200 font-bold text-slate-600 hover:border-psycho-500 hover:text-psycho-600 hover:bg-psycho-50 transition-all cursor-pointer bg-white shadow-sm";
