document.addEventListener('DOMContentLoaded', () => {
    ...
    if (typeof discussionNounsData === 'undefined') {
        console.error("discussion_nouns_data.js 로드 실패");
        return;
    }

    discussionNounsData.forEach(item => {
        ...
        container.innerHTML += `
            <div class="group bg-white p-5 rounded-2xl border border-slate-100 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 cursor-pointer flex flex-col justify-between h-40" 
                ...
                <div class="flex justify-between items-start">
                    ...
                    <span class="text-[10px] font-bold px-2 py-1 rounded-md border bg-indigo-50 text-indigo-600 border-indigo-100">
                        Nomen
                    </span>
                </div>
                <div class="text-center">
                    <div class="font-bold text-slate-800 text-lg group-hover:text-indigo-600 transition-colors mb-1 leading-tight truncate px-1">${item.w}</div>
                ...
                <div class="text-center opacity-0 group-hover:opacity-100 transition-opacity">
                        <i class="ph-bold ph-chats-circle text-indigo-400"></i>
                </div>
            </div>
        `;
    });
...
function startQuiz() {
    pool = [...discussionNounsData].sort(() => 0.5 - Math.random()).slice(0, 10);
...
function showQ() {
    ...
    const questionText = item.s.replace(new RegExp(item.w.split(' ').pop(), 'gi'), "_______");
    ...
    while(choices.length < 4) {
        let randomWord = discussionNounsData[Math.floor(Math.random()*discussionNounsData.length)].w;
        if(!choices.includes(randomWord)) choices.push(randomWord);
    }
    
    choices.sort(() => 0.5 - Math.random()).forEach(choice => {
        const btn = document.createElement('button');
        btn.className = "w-full text-left p-5 rounded-xl border border-slate-200 font-bold text-slate-600 hover:border-indigo-500 hover:text-indigo-600 hover:bg-indigo-50 transition-all cursor-pointer bg-white shadow-sm";
